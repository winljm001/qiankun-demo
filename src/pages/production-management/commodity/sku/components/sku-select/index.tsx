...
import { usePitayaNotChosenSkuOptionQuery } from '@/graphql/operations/production-management/__generated__/commodity'
...
const SkuSelect = forwardRef<SkuSelectRefProps, SkuSelectFormProps>(({ id, specData = [] }, ref) => {
  ...
  const { data: notChosenSkuOption } = usePitayaNotChosenSkuOptionQuery({ variables: { commodityId: id } })

  const data = useMemo(() => {
    return getAllSpecDescartes(currentSelectSpec, notChosenSkuOption?.pitayaNotChosenSkuOption || [])
