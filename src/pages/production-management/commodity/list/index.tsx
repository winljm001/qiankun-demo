...
import {
  ...
  usePitayaUpdateCommodityStatusMutation,
} from '@/graphql/operations/production-management/__generated__/commodity'
import { PRODUCTION_COMMODITY_ADD, PRODUCTION_COMMODITY_SKU } from '@/router/config/production-management/path'
...
const CommodityManagement = () => {
  ...
  const { tableProps, form, submit, reset, refresh } = useAsyncTable({
    gql: PitayaPageCommodityManageDocument,
    gqlKey: 'pitayaPageCommodityManage',
  })
  const [updateCommodityStatus] = usePitayaUpdateCommodityStatusMutation()
  // 修改商品状态
            ...
            actions={[
              {
                children: '管理SKU',
                onClick() {
                  history.push(`${PRODUCTION_COMMODITY_SKU}/${record?.commodityId}`)
                },
              },
            ]}
          />
      ...
      <BaseCard>
        ...
        <Space size={24}>
          <Button
            ...
            ghost
            onClick={() => {
              history.push(PRODUCTION_COMMODITY_ADD)
            }}>
            新增商品
          </Button>
        </Space>
